...
public class GraphAdjMatrix extends Graph {
     ...
     * @return List<Integer> a list of indices of vertices.
     */
    // WEI XU TRY 1: CALCULATE ALL MATRIX VALUE FIRST;
    //    public List<Integer> getDistance2(int v) {
    //        int[][] retMatrix = new int[adjMatrix.length][adjMatrix.length];
    //        for (int i = 0; i < adjMatrix.length; i++) {
    //            for (int j = 0; j < adjMatrix.length; j++) {
    //                for (int m = 0; m < adjMatrix.length; m++) {
    //                    retMatrix[i][j] += adjMatrix[i][m] * adjMatrix[m][j];
    //
    //                }
    ...
    //        List<Integer> neighbors = new ArrayList<Integer>();
    //        for (int i = 0; i < getNumVertices(); i++) {
    //            for (int j = 0; j < retMatrix[v][i]; j++) {
    //                neighbors.add(i);
    //            }
    //        }
    //        return neighbors;
    //    }
    // WEI XU TRY 2: CALCULATE only 1D array - matrix[v][i] VALUE;
    public List<Integer> getDistance2(int v) {
        int[] retMatrix = new int[adjMatrix.length];
        for (int i = 0; i < adjMatrix.length; i++) {
            retMatrix[i] = 0;
            for (int m = 0; m < adjMatrix.length; m++) {
                retMatrix[i] += adjMatrix[v][m] * adjMatrix[m][i];
            }
            System.out.print(retMatrix[i] + " ");
            
        }
        List<Integer> ret = new ArrayList<>();
        for (int i = 0; i < retMatrix.length; i++) {
            for (int j = 0; j < retMatrix[i]; j++) {
                ret.add(i);
            }
        }
        return ret;
